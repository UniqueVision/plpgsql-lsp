...
export function activate(context: ExtensionContext) {
  ...
  // --inspect=6017: runs the server in Node's Inspector mode so VS Code can attach to the server for debugging
  const debugOptions = { execArgv: ['--nolazy', '--inspect=6017'] };
  ...
  const clientOptions: LanguageClientOptions = {
    ...
    documentSelector: [{ scheme: 'file', language: 'postgres' }],
    synchronize: {
