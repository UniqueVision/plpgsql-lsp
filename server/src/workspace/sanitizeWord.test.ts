...
test("sanitize dynamic partition table", () => {
    expect(sanitizeDynamicPartitionTable('public."table_name_$$'))
        .toBe("public.table_name")

    expect(sanitizeDynamicPartitionTable('"table_name_$$'))
        .toBe("table_name")
})

test("sanitize number partition table", () => {
    expect(sanitizeNumberPartitionTable("public.table_name_1234"))
        .toBe("public.table_name")

    expect(sanitizeNumberPartitionTable("table_name_1234"))
        .toBe("table_name")
